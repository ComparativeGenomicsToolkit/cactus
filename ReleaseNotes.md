...
- `minipoa` added as an optional third base aligner in BAR, selected with `<bar baseAligner="minipoa">` in the config. abPOA remains the default. On the evolver tests the two are equivalent: primates 0.9974 against abPOA's 0.9973, mammals 0.8506 against 0.8513 -- at roughly half the peak memory (1.0 GB against 2.3 GB on evolver mammals).
