...
- Two guards against multi-Mb "hollow" snarls made by a single contig, both in the pangenome join's upstream tools (hal2vg): `filter-paf-deletions -M/-S` (`delFilterMaxRemove`/`delFilterMinSupport` in `<graphmap>`) resolves an implied deletion that only one contig asserts within an absolute budget, where the relative threshold had left a misplaced paralog in as a 42 Mb join; `clip-vg -I` (`clipInversionMin` in `<graphmap_join>`) severs an inversion allele whose junction survived clipping at one end only, which otherwise dead-ends into the reverse strand and opens a snarl the width of the inversion.
- hal2vg and its tools (clip-vg, halRemoveDupes, halMergeChroms, halUnclip, filter-paf-deletions) are now built from a pinned commit in `downloadPangenomeTools` instead of downloaded from a hal2vg release. Building it needs the HDF5 C++ wrapper (`h5c++`), cmake, autoconf and the jansson, lz4, zstd and Boost headers (`libjansson-dev liblz4-dev libzstd-dev libboost-dev` on Ubuntu), which the release-download path did not.
...
- `<bar baseAligner>` replaces the `<bar partialOrderAlignment>` boolean as the way to choose between pecan, abpoa and minipoa. The boolean is no longer read: a config that selected pecan with `partialOrderAlignment="0"` needs `baseAligner="pecan"` instead, and a config with neither gets abpoa.
