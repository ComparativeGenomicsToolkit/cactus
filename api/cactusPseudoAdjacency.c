...
PseudoAdjacency *pseudoAdjacency_loadFromBinaryRepresentation(void **binaryString, PseudoChromosome *pseudoChromosome) {
	...
	if(binaryRepresentation_peekNextElementType(*binaryString) == CODE_PSEUDO_ADJACENCY) {
		...
		_5End = net_getEnd(reference_getNet(pseudoChromosome_getReference(pseudoChromosome)), binaryRepresentation_getName(binaryString));
		_3End = net_getEnd(reference_getNet(pseudoChromosome_getReference(pseudoChromosome)), binaryRepresentation_getName(binaryString));
