...
void testPrintCanonicalReference(CuTest *testCase) {
	...
	while((pC = reference_getNextPseudoChromosome(pseudoChromosomeIterator)) != NULL) {
		...
		PseudoAdjacency *pA;
		st_logDebug("Printing pseudo chromosome %s\n", cactusMisc_nameToStringStatic(pseudoChromosome_getName(pC)));
		st_logDebug("The 5 end of the pseudo-chromosome %s\n", cactusMisc_nameToStringStatic(end_getName(pseudoChromosome_get5End(pC))));
		while((pA = pseudoChromosome_getNextPseudoAdjacency(pseudoAdjacencyIterator)) != NULL) {
			st_logDebug("Printing pseudo adjacency %s\n", cactusMisc_nameToStringStatic(pseudoAdjacency_getName(pA)));
			st_logDebug("The pseudo adjacency containing 5 end: %s", cactusMisc_nameToStringStatic(end_getName(pseudoAdjacency_get5End(pA))));
			st_logDebug("and 3 end %s\n", cactusMisc_nameToStringStatic(end_getName(pseudoAdjacency_get3End(pA))));
		}
		st_logDebug("The 3 end of the pseudo-chromosome %s\n", cactusMisc_nameToStringStatic(end_getName(pseudoChromosome_get3End(pC))));
