...
int main(int argc, char *argv[]) {
	...
	cactusDisk = cactusDisk_construct2(kvDatabaseConf, false, argv[3], true);
	st_logInfo("Set up the flower disk\n");
	flower = cactusDisk_getFlower(cactusDisk, cactusMisc_stringToName(argv[4]));
	assert(flower != NULL);
	st_logInfo("Read the flower\n");
	int64_t chunkSize, chunkOverlapSize, minimumSequenceLength;
	int64_t i = sscanf(argv[5], "%" PRIi64 "", &chunkSize);
	(void)i;
    assert(i == 1);
    i = sscanf(argv[6], "%" PRIi64 "", &chunkOverlapSize);
    assert(i == 1);
	i = sscanf(argv[7], "%" PRIi64 "", &minimumSequenceLength);
	assert(i == 1);
	setupToChunkSequences(chunkSize, chunkOverlapSize, argv[8]);
