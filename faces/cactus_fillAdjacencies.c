			  ...
			  table)
{
	...
	if (cap_getEnd(vote->candidates[index]) != end)
	    continue;
	     ...
	     (vote->candidates[index], cap_getPositiveOrientation(cap), table))
		      ...
		      Cap * generation)
{
	...
	vote->candidates[index] =
	    cap_getPositiveOrientation(adjacencyVote_getWitnessAncestor(vote->candidates
			     [index], event));
			 ...
			 table)
{
	...
	return (AdjacencyVote *) hashtable_search(table->table,
			      (void *)
			      cap_getPositiveOrientation(cap));
    else
	return NULL;
}
		      ...
		      AdjacencyVote * vote)
{
    ...
    old_vote = hashtable_remove(table->table,
			 (void *) cap,
			 false);
		      ...
		      AdjacencyVoteTable * table)
{
	...
	fillingIn_propagateAdjacencyDownwards(array[childIndex],
			      partner, table);
    }
...
static void fillingIn_pairUpToNullStub(Cap * cap, AdjacencyVoteTable * table) {
    Cap * stub = fillingIn_constructStub(cap_getEvent(cap), end_getGroup(cap_getEnd(cap)));
...
static bool fillingIn_eventsAreComparable(Event * A, Event * B) {
    return A == B
	   || event_isAncestor(A,B)
	   || event_isAncestor(B,A);
} 
...
static void fillingIn_pullDown(Cap * A)
{
	...
	for (indexB = 0; indexB < cap_getChildNumber(B); indexB++) {
	    ...
	    if ((result = fillingIn_eventsAreComparable(eventChildA, eventChildB)))
		break;
	}
...
static void fillingIn_testForPullDown(Cap * cap) {
	...
	// Derived branch:
	if (adjacencyAncestor != cap_getAdjacency(cap) && ++inconsistencyCount > 1) {
	    fillingIn_pullDown(cap);
	    break;
	}
    }
