...
void constructSpanningTree(int32_t numberOfSequences,
    ...
    while (stList_length(list) > 1) {
        ...
        assert(l != j);
        stIntTuple *m = j < l ? stIntTuple_construct(2, j, l) : stIntTuple_construct(2, l, j);
...
static int32_t *calculateIndelProbs(stList *alignedPairs, int32_t sequenceLength,
...
static void calculateIndelProbsAndAddToHash(stList *alignedPairs, int32_t sequence1Length,
        int32_t sequence1, int32_t sequence2, stHash *indelProbsHash, int32_t sequenceIndex) {
    stIntTuple *i = stIntTuple_construct(2, sequence1, sequence2);
    assert(stHash_search(indelProbsHash, i) == NULL);
    stHash_insert(indelProbsHash, i, calculateIndelProbs(alignedPairs, sequence1Length, sequenceIndex));
}

static int32_t getIndelProb(int32_t sequence1, int32_t position1, int32_t sequence2, stHash *indelProbsHash) {
    stIntTuple *i = stIntTuple_construct(2, sequence1, sequence2);
...
stList *makeAlignment(stList *sequences,
    ...
    //Indel probs hash
    stHash *indelProbsHash = stHash_construct3((uint32_t (*)(const void *))stIntTuple_hashKey, (int (*)(const void *, const void *))stIntTuple_equalsFn, (void (*)(void *))stIntTuple_destruct, free);
    ...
    while ((pairwiseAlignment = (stIntTuple *)stSortedSet_getNext(pairwiseAlignmentsIterator))
        ...
        //Make indel probs
        calculateIndelProbsAndAddToHash(alignedPairs2, strlen(stList_get(sequences, sequence1)),
                sequence1, sequence2, indelProbsHash, 0);
        calculateIndelProbsAndAddToHash(alignedPairs2, strlen(stList_get(sequences, sequence2)),
                        sequence2, sequence1, indelProbsHash, 1);
    ...
    while (stList_length(alignedPairs) > 0) {
        ...
        if (stPosetAlignment_isPossible(posetAlignment, sequence1, position1, sequence2, position2)) {
            int32_t indelProb1 = getIndelProb(sequence1, position1, sequence2, indelProbsHash);
            int32_t indelProb2 = getIndelProb(sequence2, position2, sequence1, indelProbsHash);
            if(score >= 0.1 * indelProb1 && score >= 0.1 * indelProb2) {
