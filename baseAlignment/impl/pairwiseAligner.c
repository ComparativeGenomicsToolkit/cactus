...
Diagonal bandIterator_getNext(BandIterator *bandIterator) {
    Diagonal diagonal = bandIterator->band->diagonals[bandIterator->index
            > bandIterator->band->lXalY ? bandIterator->band->lXalY
            : bandIterator->index];
...
double state_endStateProb(State state) {
    ...
    state_check(state);
    static const double endStates[5] = { TRANSITION_MATCH_CONTINUE,
            TRANSITION_MATCH_FROM_SHORT_GAP, TRANSITION_MATCH_FROM_SHORT_GAP,
            TRANSITION_MATCH_FROM_LONG_GAP, TRANSITION_MATCH_FROM_LONG_GAP };
...
DpDiagonal *dpDiagonal_construct(Diagonal diagonal) {
    ...
    dpDiagonal->cells = st_malloc(
            sizeof(double) * STATE_NUMBER * (int64_t) diagonal_getWidth(
                    diagonal));
    return dpDiagonal;
}
...
bool dpDiagonal_equals(DpDiagonal *diagonal1, DpDiagonal *diagonal2) {
    ...
    for (int32_t i = 0; i < diagonal_getWidth(diagonal1->diagonal)
            * STATE_NUMBER; i++) {
...
void dpDiagonal_zeroValues(DpDiagonal *diagonal) {
    for (int32_t i = 0; i < diagonal_getWidth(diagonal->diagonal)
            * STATE_NUMBER; i++) {
...
void dpDiagonal_initialiseValues(DpDiagonal *diagonal,
    ...
    for (int32_t i = diagonal_getMinXmy(diagonal->diagonal); i
            <= diagonal_getMaxXmy(diagonal->diagonal); i += 2) {
...
double dpDiagonal_dotProduct(DpDiagonal *diagonal1, DpDiagonal *diagonal2) {
    ...
    while (xmy <= diagonal_getMaxXmy(diagonal)) {
        totalProbability = logAdd(
                totalProbability,
                cell_dotProduct(dpDiagonal_getCell(diagonal1, xmy),
                        dpDiagonal_getCell(diagonal2, xmy)));
        xmy += 2;
    }
...
static void diagonalCalculation(
        ...
        DpDiagonal *dpDiagonalM1,
        DpDiagonal *dpDiagonalM2,
    ...
    while (xmy <= diagonal_getMaxXmy(diagonal)) {
        ...
        double *lower = dpDiagonalM1 == NULL ? NULL : dpDiagonal_getCell(
                dpDiagonalM1, xmy - 1);
        double *middle = dpDiagonalM2 == NULL ? NULL : dpDiagonal_getCell(
                dpDiagonalM2, xmy);
        double *upper = dpDiagonalM1 == NULL ? NULL : dpDiagonal_getCell(
                dpDiagonalM1, xmy + 1);
...
void diagonalCalculationForward(int32_t xay, DpMatrix *dpMatrix,
    ...
    diagonalCalculation(dpMatrix_getDiagonal(dpMatrix, xay),
            dpMatrix_getDiagonal(dpMatrix, xay - 1),
            dpMatrix_getDiagonal(dpMatrix, xay - 2), sX, sY,
...
double diagonalCalculationTotalProbability(int32_t xay,
    ...
    double totalProbability = dpDiagonal_dotProduct(forwardDiagonal,
            backDiagonal);
    ...
    if (backDiagonal != NULL && forwardDiagonal != NULL) {
        ...
        diagonalCalculation(matchDiagonal, NULL, forwardDiagonal, sX, sY,
                cell_calculateForward);
        totalProbability = logAdd(totalProbability,
                dpDiagonal_dotProduct(matchDiagonal, backDiagonal));
    ...
    while (xmy <= diagonal_getMaxXmy(diagonal)) {
        ...
        if (x > 0 && y > 0) {
            ...
            double posteriorProbability = exp(
                    (cellForward[match] + cellBackward[match])
                            - totalProbability);
#ifdef BEN_DEBUG
            ...
            if (posteriorProbability >= threshold) {
                ...
                posteriorProbability = floor(
                        posteriorProbability * PAIR_ALIGNMENT_PROB_1);

                stList_append(
                        alignedPairs,
                        stIntTuple_construct(3, (int32_t) posteriorProbability,
                                x - 1, y - 1));
            }
        }
        xmy += 2;
...
stList *getAlignedPairsWithBanding(stList *anchorPairs, const SymbolString sX,
        const SymbolString sY, PairwiseAlignmentParameters *p) {
    //Prerequisites
    ...
    Band *band = band_construct(anchorPairs, sX.length, sY.length,
            p->diagonalExpansion);
    ...
    dpDiagonal_initialiseValues(
            dpMatrix_createDiagonal(forwardDpMatrix,
                    bandIterator_getNext(forwardBandIterator)),
    ...
    while (1) { //Loop that moves through the matrix forward
        ...
        diagonalCalculationForward(diagonal_getXay(diagonal), forwardDpMatrix,
                sX, sY);

        bool atEnd = diagonal_getXay(diagonal) == diagonalNumber; //Condition true at the end of the matrix
        bool tracebackPoint = diagonal_getXay(diagonal) >= tracedBackTo
        ...
        if (atEnd || tracebackPoint) {
            ...
            int32_t tracedBackFrom = diagonal_getXay(diagonal) - (atEnd ? 0
            ...
            while (diagonal_getXay(diagonal2) > tracedBackTo) {
                ...
                if (diagonal_getXay(diagonal2) > tracedBackTo + 1) {
                    diagonalCalculationBackward(diagonal_getXay(diagonal2),
                ...
                if (diagonal_getXay(diagonal2) <= tracedBackFrom) {
                    ...
                    diagonalCalculationPosteriorMatchProbs(
                            diagonal_getXay(diagonal2), forwardDpMatrix,
                            backwardDpMatrix, p->threshold, totalProbability,
                            alignedPairs);

                    if (diagonal_getXay(diagonal2) < tracedBackFrom || atEnd) {
                        dpMatrix_deleteDiagonal(forwardDpMatrix,
                                diagonal_getXay(diagonal2)); //Delete forward diagonal after last access in posterior calculation
                    }
                }
                if (diagonal_getXay(diagonal2) + 1 <= diagonalNumber) {
                    dpMatrix_deleteDiagonal(backwardDpMatrix,
                            diagonal_getXay(diagonal2) + 1); //Delete backward diagonal after last access in backward calculation
                }
            ...
            totalPosteriorCalculations
                    += totalPosteriorCalculationsThisTraceback;
            if (!atEnd) {
                assert(
                        dpMatrix_getActiveDiagonalNumber(forwardDpMatrix)
                                == p->traceBackDiagonals + 2);
            }
        }
...
static void convertBlastPairs(stList *alignedPairs2, int32_t offsetX,
        int32_t offsetY) {
    /*
...
static void getBlastPairsForPairwiseAlignmentParametersP(const char *sX,
    ...
    if (matrixSize > p->repeatMaskMatrixBiggerThanThis) {
        ...
        stList *unfilteredBottomLevelAnchorPairs = getBlastPairs(sX2, sY2, lX2,
                lY2, p->constraintDiagonalTrim, 0);
        stList_sort(unfilteredBottomLevelAnchorPairs,
                (int(*)(const void *, const void *)) stIntTuple_cmpFn);
        stList *bottomLevelAnchorPairs = getAnchorChain(
                unfilteredBottomLevelAnchorPairs, p->alpha);
        st_logDebug(
                "Got %i bottom level anchor pairs, which reduced to %i after filtering \n",
                stList_length(unfilteredBottomLevelAnchorPairs),
...
stList *getBlastPairsForPairwiseAlignmentParameters(const char *sX,
    ...
    stList *unfilteredTopLevelAnchorPairs = getBlastPairs(sX, sY, lX, lY,
            p->constraintDiagonalTrim, 1);
    stList_sort(unfilteredTopLevelAnchorPairs,
            (int(*)(const void *, const void *)) stIntTuple_cmpFn);
    stList *topLevelAnchorPairs = getAnchorChain(unfilteredTopLevelAnchorPairs,
            p->alpha);
    st_logDebug(
            "Got %i top level anchor pairs, which reduced to %i after filtering \n",
            stList_length(unfilteredTopLevelAnchorPairs),
...
static void getSplitPointsP(int32_t *x1, int32_t *y1, int32_t x2, int32_t y2,
        int32_t x3, int32_t y3, stList *splitPoints,
        int64_t splitMatrixBiggerThanThis) {
    int32_t lX2 = x3 - x2;
    int32_t lY2 = y3 - y2;
    int64_t matrixSize = ((int64_t) lX2) * lY2;
    if (matrixSize > splitMatrixBiggerThanThis) {
        ...
        stList_append(splitPoints,
                stIntTuple_construct(4, *x1, *y1, x2 + hX, y2 + hY));
        *x1 = x3 - hX;
        *y1 = y3 - hY;
    }
}
...
stList *getSplitPoints(stList *anchorPairs, int32_t lX, int32_t lY,
    ...
    for (int32_t i = 0; i < stList_length(anchorPairs); i++) {
        ...
        int32_t x3 = stIntTuple_getPosition(anchorPair, 0), y3 =
    ...
    getSplitPointsP(&x1, &y1, x2, y2, lX, lY, splitPoints,
            splitMatrixBiggerThanThis);
    ...
    if (stList_length(splitPoints) > 1) {
        st_logDebug("For sequences of length %i and %i we got %i splits\n", lX,
                lY, stList_length(splitPoints));
    }
    return splitPoints;
}

static void convertAlignedPairs(stList *alignedPairs2, int32_t offsetX,
        int32_t offsetY) {
    /*
     * Convert the coordinates of the computed pairs.
     */
...
stList *splitAlignmentsByLargeGaps(stList *anchorPairs, const char *sX,
    ...
    for (int32_t i = 0; i < stList_length(splitPoints); i++) {
        ...
        stList *subListOfAnchorPoints = stList_construct3(0,
                (void(*)(void *)) stIntTuple_destruct);
        while (j < stList_length(anchorPairs)) {
            ...
            assert(x >= x1 && x < x2);
            assert(y >= y1 && y < y2);
            stList_append(subListOfAnchorPoints,
                    stIntTuple_construct(2, x - x1, y - y1));
            j++;
        }

        //Make the alignments
        stList *subListOfAlignedPairs = getAlignedPairsWithBanding(
                subListOfAnchorPoints, sX3, sY3, p);
