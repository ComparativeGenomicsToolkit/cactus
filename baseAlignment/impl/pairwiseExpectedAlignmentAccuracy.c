...
static int64_t *getCummulativeIndelProbs(int32_t *indelProbs, int32_t length) {
    ...
    assert(length >= 0);
#endif
    int64_t *cummulativeIndelProbs = st_malloc(sizeof(int64_t) * length);
    if(length > 0) {
        cummulativeIndelProbs[0] = indelProbs[0];
        for(int32_t i=1; i<length; i++) {
            cummulativeIndelProbs[i] = indelProbs[i] + cummulativeIndelProbs[i-1];
        }
    }
...
stHash *getExpectedAlignmentAccuracyScores(stList *alignedPairs, int32_t *indelProbs1, int32_t *indelProbs2, int32_t seqLength1, int32_t seqLength2) {
    ...
    for(int32_t i=0; i<stList_length(alignedPairs); i++) {
        ...
        d[0] = (((double)forwardScore[0] + reverseScore - stIntTuple_getPosition(alignedPair, 0)) / (seqLength1 + seqLength2)) / PAIR_ALIGNMENT_PROB_1; //We subtract the aligned pair score count so as not to count it twice.
#ifdef BEN_DEBUG
        assert(d[0] >= -0.00001);
        assert(d[0] <= 1.00001);
#endif
