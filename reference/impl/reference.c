    ...
    while ((end = flower_getNextEnd(endIt)) != NULL) {
        ...
        if (end_isAttached(end)) {
            ...
            if (adjacencyPair_getEnd1(adjacencyPair) == end) { //Just build the pseudo chromosome once..
                ...
                PseudoChromosome *pseudoChromosome = pseudoChromosome_construct(reference, end, end2);
...
static void balanceTanglesRecursively(Flower *flower, void(*fn)(Flower *)) {
    /*
    ...
    if (!flower_isTerminal(flower)) {
        ...
        while ((group = flower_getNextGroup(groupIt)) != NULL) {
            ...
            if (group_isTangle(group)) {
                balanceTanglesRecursively(group_getNestedFlower(group), fn);
            }
        }
...
void constructReference_topDownPhase(Flower *flower, MatchingAlgorithm matchingAlgorithm) {
     ...
     */
    balanceTanglesRecursively(flower, balanceTangles);
    /*
     * Breaks any circles.
     */
    balanceTanglesRecursively(flower, breakCircles);
...
void constructReference_bottomUpPhase(Flower *flower) {
    if (flower_isTerminal(flower)) {
        ...
        Flower_EndIterator *endIt = flower_getEndIterator(flower);
        while ((end = flower_getNextEnd(endIt)) != NULL) {
            if (end_isAttached(end) && end_isStubEnd(end) && end_getPseudoAdjacency(end) == NULL) {
                ...
                do { //This loop constructs a pseudo chromosome..
                    ...
                    PseudoAdjacency *nestedPseudoAdjacency = end_getPseudoAdjacency(nestedEnd);
                    assert(nestedPseudoAdjacency != NULL);
                    PseudoChromosome *nestedPseudoChromosome = pseudoAdjacency_getPseudoChromosome(
                            nestedPseudoAdjacency);
                    assert(pseudoChromosome_get5End(nestedPseudoChromosome) == nestedEnd || pseudoChromosome_get3End(
                            nestedPseudoChromosome) == nestedEnd);
                    End *otherNestedEnd =
                            pseudoChromosome_get5End(nestedPseudoChromosome) == nestedEnd ? pseudoChromosome_get3End(
                                    nestedPseudoChromosome) : pseudoChromosome_get5End(nestedPseudoChromosome);
                    assert(otherNestedEnd != NULL);
                    assert(nestedEnd != otherNestedEnd);
                    End *otherEnd = flower_getEnd(flower, end_getName(otherNestedEnd));
                 ...
                 */
                PseudoChromosome *pseudoChromosome = pseudoChromosome_construct(reference, stList_get(list, 0),
                        stList_peek(list));
                for (int32_t i = 0; i < stList_length(list); i += 2) {
                    pseudoAdjacency_construct(stList_get(list, i), stList_get(list, i + 1), pseudoChromosome);
                }
