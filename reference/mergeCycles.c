...
void mergeCycles(stHash *adjacencies, Flower *flower) {
    ...
    stList *components = adjacencyHash_getConnectedComponents(adjacencies, flower);
    ...
    while (stList_length(cycles) > 0) { //Now merge the cycles
    	...
    	for(int32_t j=0; j<stList_length(components); j++) {
    		...
    		if(cycle != component) {
    			AdjacencySwitch *adjacencySwitch2 = adjacencySwitch_getStrongestAdjacencySwitch(cycle, component);
    			if(adjacencySwitch2 != NULL) {
    				if(adjacencySwitch == NULL || adjacencySwitch_cmpByStrength(adjacencySwitch2, adjacencySwitch) >= 0) {
    	...
    	stList *newComponent = adjacencyHash_getConnectedComponent(adjacencies, flower, adjacencyPair_getEnd1(adjacencySwitch_getAdjacencyPair1(adjacencySwitch)));
    	...
    	adjacencyPair_destruct(adjacencySwitch_getAdjacencyPair1(adjacencySwitch));
    	adjacencyPair_destruct(adjacencySwitch_getAdjacencyPair2(adjacencySwitch));
    	adjacencySwitch_destruct(adjacencySwitch);
