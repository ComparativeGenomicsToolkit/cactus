...
static double adjacencyPair_getStrengthOfAdjacencyPair(
...
static int adjacencyPair_cmpFnByStrength(AdjacencyPair **adjacencyPair1,
...
static int adjacencyPair_hashEqual(AdjacencyPair *adjacencyPair1,
        AdjacencyPair *adjacencyPair2) {
    /*
...
static struct List *makeListOfAdjacencyPairs(Flower *flower) {
    /*
...
static void addAdjacencyPairToHash(stHash *adjacenciesHash,
        AdjacencyPair *adjacencyPair) {
    /*
...
static void removeAndDestructAdjacency(stHash *adjacencies,
        AdjacencyPair *adjacencyPair) {
    /*
     * Removes the adjacency pair from the hash and destroys it.
     */
    assert(stHash_remove(adjacencies, adjacencyPair_getEnd1(adjacencyPair)) == adjacencyPair);
    assert(stHash_remove(adjacencies, adjacencyPair_getEnd2(adjacencyPair)) == adjacencyPair);
    adjacencyPair_destruct(adjacencyPair);
}

static stHash *choosePairing(struct List *adjacencies, Flower *flower) {
    /*
...
void adjacenciesHash_cleanUp(Flower *flower, stHash *adjacencies) {
    /*
...
static void extendComponent(End *end, struct List *component,
...
struct List *getConnectedComponents(stHash *adjacencies, Flower *flower) {
    /*
     * Gets a list of connected components of ends linked by adjacencies and blocks.
     */
    struct List *components = constructEmptyList(0,
    ...
    while ((end = flower_getNextEnd(endIterator)) != NULL) { //iterates over the positive oriented ends.
        if (end_isBlockEnd(end) || end_isAttached(end)) { //ignore free stubs
            struct List *component = stHash_search(componentsHash, end);
            if (component == NULL) {
...
void getAttachedStubEndsInComponent(struct List *component, End **end1,
        End **end2) {
    /*
...
void switchAdjacencies(AdjacencyPair *adjacencyPair1,
        AdjacencyPair *adjacencyPair2, stHash *adjacencies,
...
void splitIntoContigsAndCycles(struct List *components, struct List **contigs,
        struct List **cycles) {
    /*
...
AdjacencyPair *getAdjacencyPair(struct List *component, stHash *adjacencies) {
    ...
    AdjacencyPair *adjacencyPair = stHash_search(adjacencies,
            component->list[0]);
...
void mergeCycles(stHash *adjacencies, Flower *flower) {
    ...
    struct List *components = getConnectedComponents(adjacencies, flower);
    ...
    for (i = 0; i < cycles->length; i++) { //Now merge the cycles
        ...
        switchAdjacencies(adjacencyPair1, adjacencyPair2, adjacencies,
                &adjacencyPair3, &adjacencyPair4);
...
static stHash *getCorrectEndPairing(Reference *reference) {
     ...
     */
    stHash *correctEndPairing = stHash_construct();
...
void breakApartMisPairedContigs(stHash *correctEndPairing, stHash *adjacencies,
        Flower *flower) {
    /*
    ...
    for (i = 0; i < components->length; i++) { //Get mispaired contigs, breaking them up.
        ...
        assert(end1 != NULL);
        assert(end2 != NULL);
        AdjacencyPair *adjacencyPair1 = stHash_search(correctEndPairing, end1); //defensive
        AdjacencyPair *adjacencyPair2 = stHash_search(correctEndPairing, end2);
        if (adjacencyPair1 != adjacencyPair2) {
            //Break an adjacency
            removeAndDestructAdjacency(adjacencies, getAdjacencyPair(component,
                    adjacencies));
        }
    }
    destructList(components);
...
End *getFreeEnd(struct List *component, stHash *adjacencies) {
    ...
    for (i = 0; i < component->length; i++) {
        ...
        if (stHash_search(adjacencies, end2) == NULL) {
            assert(end == NULL);
            end = end2;
        }
    }
...
void pairBrokenContigs(stHash *correctEndPairing, stHash *adjacencies,
        Flower *flower) {
    /*
    ...
    for (i = 1; i < components->length; i++) { //Get mispaired contigs, breaking them up.
        ...
        if (end2 == NULL) { //get the other component
            AdjacencyPair *adjacencyPair1 = stHash_search(correctEndPairing,
                    end1);
            for (j = 0; j < i; j++) {
                End *end3, *end4;
                getAttachedStubEndsInComponent(components->list[j], &end3,
                        &end4);
                assert(end3 != NULL);
                if (end4 == NULL) {
                    AdjacencyPair *adjacencyPair2 = stHash_search(
                            correctEndPairing, end3);
                    if (adjacencyPair1 == adjacencyPair2) {
                        addAdjacencyPairToHash(adjacencies,
                                adjacencyPair_construct(getFreeEnd(
                                        components->list[i], adjacencies),
                                        getFreeEnd(components->list[j],
                                                adjacencies)));
                    }
                }
            }
...
void correctAttachedStubEndPairing(stHash *adjacencies, Flower *flower,
    ...
    breakApartMisPairedContigs(correctEndPairing, adjacencies, flower);
    pairBrokenContigs(correctEndPairing, adjacencies, flower);
    adjacenciesHash_cleanUp(flower, correctEndPairing);
}
...
static void fillInPseudoAdjacenciesP(End *end1,
    ...
    if (group1 != group2) {
        ...
        End *end3 = block_get5End(pseudoBlock), *end4 = block_get3End(
                pseudoBlock);
        ...
        pushEndIntoChildFlowers(end3);
        pushEndIntoChildFlowers(end4);
...
void makePseudoAdjacencies(Flower *flower, Reference *reference) {
     ...
     */
    struct List *adjacencies = makeListOfAdjacencyPairs(flower);
    qsort(adjacencies->list, adjacencies->length, sizeof(void *), (int(*)(
            const void *v, const void *)) adjacencyPair_cmpFnByStrength);
    ...
    adjacenciesHash_cleanUp(flower, adjacenciesHash);
}
