...
void testBlock_makeNewickString(CuTest *testCase) {
	cactusBlockTestSetup();
	char *cA = block_makeNewickString(block, 1, 0);
...
void testBlock_serialisation(CuTest* testCase) {
	...
	int32_t i;
	void *vA = binaryRepresentation_makeBinaryRepresentation(block,
			(void (*)(void *, void (*)(const void *, size_t, size_t)))block_writeBinaryRepresentation, &i);
	CuAssertTrue(testCase, i > 0);
	block_destruct(block);
	void *vA2 = vA;
	block = block_loadFromBinaryRepresentation(&vA2, net);
	rootSegment = segment_getReverse(block_getInstance(block, rootInstanceName));
	leaf1Segment = block_getInstance(block, leaf1InstanceName);
	leaf2Segment = segment_getReverse(block_getInstance(block, leaf2InstanceName));
	free(vA);
	nestedTest = 1;
