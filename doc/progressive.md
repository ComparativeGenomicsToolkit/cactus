...
* `--doubleTime true` (highly recommended): the time analogue of `--doubleMem`.  If slurm kills a job for running past its walltime, retry it with double the walltime.  This is what makes the per-job estimates above safe to keep tight: an occasional underestimate costs one retry rather than the run.  (A job with no walltime at all -- `--defaultWalltime 0` -- has nothing to double, so this only helps once the times above are set.)
* `--slurmTime` is a global override that forces a single time onto *every* job, ignoring the per-job `--defaultWalltime` / `--fastWalltime` values above (so it also disables the fast-partition routing, and `--doubleTime` along with it).  Reach for it only as an escape hatch, e.g. if a job's time estimate turns out too low and it keeps getting killed: `--slurmTime 200:00:00`.
