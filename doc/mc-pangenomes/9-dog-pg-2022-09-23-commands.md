...
cactus-graphmap-join ${MYJOBSTORE} --vg $(for j in $(for i in `seq 38`; do echo chr$i; done ; echo "chrX chrM chrOther"); do echo ${MYBUCKET}/clip-dog-pg-${VERSION}/${j}.vg; done) --outDir ${MYBUCKET} --outName dog-pg-${VERSION}-minaf.0.22 --reference canFam4  --wlineSep "." --vgClipOpts "-d 2 -m 1000" --preserveIDs --giraffe --logFile dog-pg-${VERSION}-minaf.0.22.log --batchSystem mesos --provisioner aws --defaultPreemptable --nodeType r5.16xlarge --nodeStorage 1000 --maxNodes 2 --indexCores 64 --realTimeLogging
