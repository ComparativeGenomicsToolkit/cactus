...
cactus-graphmap-join ${MYJOBSTORE} --vg $(for j in $(for i in `seq 29`; do echo chr$i; done ; echo "chrX chrM chrOther"); do echo ${MYBUCKET}/clip-cow-pg-${VERSION}/${j}.vg; done) --outDir ${MYBUCKET} --outName cow-pg-${VERSION}-minaf.0.4 --reference bosTau9  --wlineSep "." --vgClipOpts "-d 2 -m 1000" --preserveIDs --giraffe --logFile cow-pg-${VERSION}-join-minaf.0.4.log --batchSystem mesos --provisioner aws --defaultPreemptable --nodeType r5.8xlarge:1.5 --nodeStorage 1000 --maxNodes 2 --indexCores 32 --realTimeLogging
