...
cactus-graphmap-join ${MYJOBSTORE} --vg $(for j in $(for i in `seq 20`; do echo chr$i; done ; echo "chrM chrOther"); do echo ${MYBUCKET}/clip-soybean-pg-${VERSION}/${j}.vg; done) --outDir ${MYBUCKET} --outName soybean-pg-${VERSION}-minaf.0.12 --reference Glycine_max_v4_0  --wlineSep "." --vgClipOpts "-d 2 -m 1000" --preserveIDs --giraffe --logFile soybean-pg-${VERSION}-minaf.0.12.log --batchSystem mesos --provisioner aws --defaultPreemptable --nodeType r5.8xlarge:1.5 --nodeStorage 1000 --maxNodes 2 --indexCores 32 --realTimeLogging
