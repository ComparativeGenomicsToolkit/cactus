...
One implementation note, in case the `LV` values surprise you: `vg` counts `LV` within each reference contig, not as absolute depth in the snarl tree. A gref site is therefore at `LV` 0 when nothing on its *own* contig encloses it, however deeply the enclosing main-reference bubble is nested. Both halves of the VCF consequently go through one `vcfbub --max-level 0` pass together. `PS` is unaffected — it still names the parent snarl wherever that snarl lies, which is what lets `vcfbub` rescue the children of bubbles it pops.
