...
int32_t checkCap(XMLNode endNode, struct hashtable *validEvents,
	...
	assert(strcmp(endTreeNode.getChildNode("clade").getAttribute("event"), rootEventName) == 0); //check that the root event in an end tree is the 'root' event of the event tree for the reconstruction.
...
void checkCapTreeAdjacenciesP(XMLNode parentCapInstanceNode, XMLNode capInstanceNode,
	...
	assert(strcmp((char *)hashtable_search(adjacencyMap, (void *)adjacency), capInstanceNode.getAttribute("instance")) == 0); //checks that the adjacency the cap instance has is linked back to it.

	//check adjacency map is okay
	assert(hashtable_search(parentMap, (void *)adjacency) != NULL);
	...
	assert(strcmp((char *)hashtable_search(adjacencyMap, (void *)parentAdjacency), parentCapInstanceNode.getAttribute("instance")) == 0); //checks that the adjacency the cap instance has is linked back to it.
	...
	if(!capInstanceNode.isAttributeSet("operation_index")) { //no operation
		...
		assert(strcmp((char *)hashtable_search(parentMap, (void *)adjacency), parentAdjacency) == 0); //check that parent and child end instances have an equivalent adjacency, given that there is no operation.
	}
	else { //has an op
	...
	for(i=0; i<stringsNode.nChildNode("string"); i++) {
		...
		if(stringNode.getText() != NULL) {
			...
			do {
				...
				assert(strcmp((char *)hashtable_search(adjacencyMap, cA3), cA4) == 0);
				assert(strcmp((char *)hashtable_search(adjacencyMap, cA4), cA3) == 0);
				free(cA3);
				free(cA4);
				cA3 = cA5;
				cA = strtok(NULL, " ");
