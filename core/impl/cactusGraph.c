            ...
            if (lengthBlackEdges(pinchVertex) > 0) {
                ...
                if (!passThroughEdgeFn(pinchEdge)) {
                    ...
                    if (pinchVertex->vertexID < pinchVertex2->vertexID) {
                        ...
                        void *blackEdgeIterator = getBlackEdgeIterator(
                                pinchVertex);
...
struct List *readThreeEdgeComponents(struct PinchGraph *pinchGraph,
    ...
    int32_t l = 0;
#endif
    struct List *threeEdgeConnectedComponents = constructEmptyList(0,
            (void(*)(void *)) destructList);
        ...
        for (j = 0; j < stList_length(threeEdgeConnectedAdjacencyComponent); j++) {
            stSortedSet
                    *adjacencyComponent =
                            stList_get(
                                    adjacencyComponents,
                                    stIntTuple_getPosition(
                                            stList_get(
                                                    threeEdgeConnectedAdjacencyComponent,
                                                    j), 0));
...
static bool containsMultipleCopiesOfSpecies(struct CactusEdge *cactusEdge,
        Flower *flower, struct PinchGraph *pinchGraph,
     ...
     */
    stList *events = getEvents(cactusEdge, flower, pinchGraph);
    stSortedSet *ingroupEvents = stSortedSet_construct();
    stSortedSet *outgroupEvents = stSortedSet_construct();
    bool b = 0;
    while (stList_length(events) > 0) {
        Event *event = stList_pop(events);
        if (event_isOutgroup(event)) {
            if (singleCopyOutgroupSpecies) {
                if (stSortedSet_search(outgroupEvents, event) != NULL) {
                    b = 1;
                    break;
                }
                stSortedSet_insert(outgroupEvents, event);
            }
        } else {
            if (singleCopyIngroupSpecies) {
                if (stSortedSet_search(ingroupEvents, event) != NULL) {
                    b = 1;
                    break;
                }
                stSortedSet_insert(ingroupEvents, event);
            }
        }
    }
    stList_destruct(events);
    stSortedSet_destruct(ingroupEvents);
    stSortedSet_destruct(outgroupEvents);
    return b;
}
...
bool chainExtendsParentChain(struct List *biConnectedComponent,
        struct PinchGraph *pinchGraph, Flower *flower) {
    assert(biConnectedComponent->length > 0);
    /*
    ...
    bool b = isAStubCactusEdge(biConnectedComponent->list[0], pinchGraph)
            && !isAFreeStubCactusEdge(biConnectedComponent->list[0],
                    pinchGraph, flower) && isAStubCactusEdge(
            biConnectedComponent->list[biConnectedComponent->length - 1],
            pinchGraph) && !isAFreeStubCactusEdge(
            biConnectedComponent->list[biConnectedComponent->length - 1],
            pinchGraph, flower);
    return b && flower_hasParentGroup(flower);
}

stSortedSet *getPinchVerticesSet(stSortedSet *cactusEdges,
        struct PinchGraph *pinchGraph) {
...
stSortedSet *filterBlocksByTreeCoverageAndLength(
    ...
    for (int32_t i = 0; i < biConnectedComponents->length; i++) {
        ...
        if (minimumChainLength <= 0 || chainBaseLength(biConnectedComponent,
                pinchGraph) >= minimumChainLength || chainExtendsParentChain(
                biConnectedComponent, pinchGraph, flower)) {
            for (int32_t j = 0; j < biConnectedComponent->length; j++) {
                struct CactusEdge *cactusEdge = biConnectedComponent->list[j];
                if (!isAStubCactusEdge(cactusEdge, pinchGraph)) {
                        ...
                        if (minimumTreeCoverage <= 0.0 || treeCoverage2(
                            ...
                            if (minimumBlockLength <= 0 || piece->end
                                ...
                                if ((requiredIngroupSpecies == 0 && requiredOutgroupSpecies == 0 && requiredAllSpecies == 0)
                                    ...
                                    if ((!singleCopyIngroupSpecies && !singleCopyOutgroupSpecies)
                                            || !containsMultipleCopiesOfSpecies(
                                                    cactusEdge, flower,
                                                    pinchGraph,
                                                    singleCopyIngroupSpecies, singleCopyOutgroupSpecies)) {
                                        stSortedSet_insert(chosenBlocks,
                                                cactusEdge);
                                    }
                                }
                            }
...
int32_t isAFreeStubCactusEdge(struct CactusEdge *edge,
    ...
    if (isAStubCactusEdge(edge, pinchGraph)) {
        ...
        assert(
                vertex_isDeadEnd(pinchEdge->from) || vertex_isDeadEnd(
                        pinchEdge->to));
        assert(
                !(vertex_isDeadEnd(pinchEdge->from) && vertex_isDeadEnd(
                        pinchEdge->to)));
