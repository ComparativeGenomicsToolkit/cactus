...
def make_plan(
    ...
    # remove old header
    plan = re.sub("## generated by", "## wrapping", plan, flags=re.IGNORECASE | re.MULTILINE)
    plan = re.sub("## date .*?\n", "", plan, flags=re.IGNORECASE | re.MULTILINE)
...
def get_plan_adding2node(
    ...
    plan = make_plan(
        ...
        patch,
        assemblies,
        [name for name in length_map if name not in in_fasta_map],
        options,
    )
...
def get_plan_adding2branch(
    ...
    plan = make_plan(
        ...
        patch,
        assemblies,
        [child_genome] + [name for name in length_map if name != patch],
    )
