...
def make_plan(
    ...
    # run cactus_prepare
    plan = call_cactus_prepare(seq_file, out_dir, jobstore_dir, out_seq_file, cactus_prepare_options)
...
def sanity_checks(options):
    ...
    def work_dir_sanity_check():
        ...
        # just create them
        os.makedirs(options.jobstore_dir, exist_ok=True)
...
def main():
    ...
    parent_parser.add_argument(
        ...
        default=None,
        help="Use the the specified pre-built containter image " "rather than pulling one from quay.io",
    )
    parent_parser.add_argument(
        "--binariesMode",
