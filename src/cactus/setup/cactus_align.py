...
def main():
    ...
    parser.add_argument("--maxLen", type=int, default=None,
                        help="Only align up to this many bases (overrides <bar bandingLimit> and <caf maxRecoverableChainLength> in configuration) [default=10000 with --pangenome]")
    ...
    parser.add_argument("--consOptions", type=str, default=None,
                        help="Extra options passed verbatim to cactus_consolidated, eg "
                        "\"--barNestedThreads 1\" or \"--barBoundThreads\". Experimental, for OpenMP tuning.")
...
def make_align_job(options, toil, config_wrapper=None, chrom_name=None):
    ...
    align_job = Job.wrapJobFn(cactus_align,
                              ...
                              cons_options=options.consOptions,
...
def cactus_align(job, config_wrapper, mc_tree, input_seq_map, input_seq_id_map, paf_id, paf_path, root_name, og_map, checkpointInfo, doVG, doGFA, delay=0,
                 referenceEvents=None, pafMaskFilter=None, paf2Stable=False, cons_cores = None, cons_memory = None, do_filter_paf=False, chrom_name=None, scores_id=None, branch_scale=1.0, cons_options=None):
...
def export_vg(job, hal_id, config_wrapper, doVG, doGFA, referenceEvents, checkpointInfo=None, resource_spec = False,
    ...
    if not resource_spec:
        ...
        # this; it is unset (-> 0, no effect) for cactus-pangenome.  cactus_clamp_memory still bounds
        # the result by --maxMemory so the floor can't make the job unschedulable
