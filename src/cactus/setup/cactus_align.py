...
def main():
    ...
    parser.add_argument("--batch", action="store_true", help = "Launch batch of alignments.  Input seqfile is expected to be chromfile as generated by cactus-graphmap-slit. IMPORTANT: pafFile argument must be specified as \"\"")
...
def make_align_job(options, toil):
    ...
    tmp_job_store = None
    if not makeURL(options.seqFile).startswith('file'):
        # import the seqfile via a temporary job store
        # todo: this seems hacky, maybe there's a simpler Toil interface?
        temp_dir = getTempDirectory()
        js_dir = os.path.join(temp_dir, 'js')
        tmp_job_store = toil.getJobStore(makeURL(js_dir))
        tmp_job_store.initialize(Config())
        tmp_job_store.import_file(makeURL(options.seqFile), shared_file_name=os.path.basename(options.seqFile))
        options.seqFile = tmp_job_store.get_shared_public_url(os.path.basename(options.seqFile)).replace('file:','')
    ...
    if options.pangenome:
        ...
        # (only when --base not passed to cactus-graphmap, but harmless otherwise)
...
def cactus_align(job, config_wrapper, mc_tree, input_seq_map, input_seq_id_map, paf_id, root_name, og_map, checkpointInfo, doVG, doGFA,
    ...
    if doVG or doGFA:
        # sending in cons_cores as a proxy for making sure it has enough memory
