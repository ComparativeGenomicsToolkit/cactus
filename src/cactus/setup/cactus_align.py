...
def cactus_align(job, config_wrapper, mc_tree, input_seq_map, input_seq_id_map, paf_id, paf_path, root_name, og_map, checkpointInfo, doVG, doGFA, delay=0,
    ...
    base_calling_tree = mc_tree if branch_scale == 1.0 else \
        get_ancestor_scaled_tree(mc_tree, root_name, None, branch_scale=branch_scale)
    findRequiredNode(config_wrapper.xmlRoot, 'reference').set('baseCallingTree', NXNewick().writeString(base_calling_tree))
...
def export_vg(job, hal_id, config_wrapper, doVG, doGFA, referenceEvents, checkpointInfo=None, resource_spec = False,
    ...
    if not resource_spec:
        ...
        # now and try again
        vg_memory = hal_id.size * 30 if not memory_override else memory_override
