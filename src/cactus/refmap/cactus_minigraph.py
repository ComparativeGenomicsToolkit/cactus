    ...
    if getOptionalAttrib(xml_node, "collapseInversions", typeFn=bool, default=False) and \
       not getattr(options, 'refOnly', False):
        ...
        collapse_job = minigraph_job.addFollowOnJobFn(collapse_inversions, options, config_node,
                                                      ...
                                                      # keyed on the same reference size the disk and
                                                      # memory now use rather than resolving the graph
                                                      # promise: the runtime is nearly all the fixed
                                                      # minimap2 tail, and rgfa-collapse itself ran
                                                      # 18-187 s over 24 HPRC chromosomes, so the
                                                      # per-byte term is insurance either way.
                                                      walltime=cactus_walltime(
                                                          collapse_inversions_walltime(collapse_ref_size, options.mgCores),
                                                          io_bytes=4 * collapse_ref_size))
...
def collapse_inversions_walltime(ref_size, cores):
    """ estimated seconds for one collapse_inversions job, from the reference its graph carries """
