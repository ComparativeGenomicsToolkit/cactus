...
from cactus.refmap.cactus_graphmap import make_mapq_track_wrapper
...
def pangenome_end_to_end_workflow(job, options, config_wrapper, seq_id_map, seq_path_map, seq_order, ref_collapse_paf_id,
    ...
    # under --mgSplit everything below is redone against single-chromosome graphs, whose MAPQ is
    # inflated because the competing chromosomes are not in the graph.  boil the whole-genome PAF
    # down to the MAPQ it gave each query interval first -- it is deleted by the cleanup job
    # immediately after this.  the raw PAF, not the filtered one: filter_paf deletes everything
    # under minMAPQ, which is the strongest evidence the floor has
    mapq_track_id = None
    if options.mgSplit and getOptionalAttrib(findRequiredNode(config_node, "graphmap"),
                                             "mgSplitMapqFloor", typeFn=bool, default=True):
        mapq_track_job = split_export_job.addFollowOnJobFn(make_mapq_track_wrapper, paf_id)
        mapq_track_id = mapq_track_job.rv()
        split_export_job = mapq_track_job
    ...
    if options.mgSplit:
        ...
        graphmap_batch_options.mg_mapq_track_id = mapq_track_id
