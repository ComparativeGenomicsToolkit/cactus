...
def graphmap_join_options(parser):
    ...
    parser.add_argument("--snarl-stats", nargs='*', help = "Write a list of snarl statistics for the graph type(s). Valid types are 'full', 'clip' and 'filter'. If no type specified, 'clip' will be used ('full' used if clipping disabled). Multipe types can be provided separated by space")
...
def graphmap_join_validate_options(options):
    ...
    if options.snarl_stats == []:
        options.snarl_stats = ['clip'] if options.clip else ['full']
    options.snarl_stats = list(set(options.snarl_stats)) if options.snarl_stats else []    
    for snarl_stats in options.snarl_stats:
        if snarl_stats not in ['clip', 'filter', 'full']:
            raise RuntimeError('Unrecognized value for --snarl-stats: {}. Must be one of {{clip, filter, full}}'.format(snarl_stats))
        if snarl_stats == 'clip' and not options.clip:
            raise RuntimError('--snarl-stats cannot be set to clip since clipping is disabled')
        if snarl_stats == 'filter' and not options.filter:
            raise RuntimeError('--snarl-stats cannot be set to filter since filtering is disabled')
...
def graphmap_join_workflow(job, options, config, vg_ids, hal_ids):
    ...
    for workflow_phase, phase_vg_ids, phase_root_job in workflow_phases:
        ...
        if workflow_phase in options.snarl_stats:
            snarl_stats_ids = []
            for vg_path, vg_id, input_vg_id in zip(options.vg, phase_vg_ids, vg_ids):
