...
//                       report intervals that are covered by any alignments
...
#define programRevisionDate    "20131122"
...
static void  emit_intervals      (FILE* f, u8* window, char* chrom,
                                  u32 pendingRun, u32 windowStart, u32 windowEnd);
static u32   emit_some_intervals (FILE* f, u8* window, char* chrom,
                                  u32 pendingRun, u32 windowStart, u32 windowEnd);
...
static void usage
   (char*	message)
	...
	fprintf (stderr, "covered by any alignments.\n");
	fprintf (stderr, "\n");
   ...
   (int		argc,
	...
	while (true)
		...
		if (strcmp (qChrom, prevChrom) != 0)
			{
			if (prevChrom[0] != 0)
				emit_intervals (stdout, window, prevChrom, pendingRun,
		...
		if (qEnd > windowStart + windowSize)
			...
			pendingRun = emit_some_intervals (stdout, window, qChrom, pendingRun,
...
static void emit_intervals
   (FILE*	f,
	...
	run = emit_some_intervals (f, window, chrom, pendingRun, windowStart, windowEnd);
	if (run > 0)
